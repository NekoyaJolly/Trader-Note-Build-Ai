...
> **【配線状況 / Step A-1 以降】**
> 本エージェントは現在 **dormant** です。Step A-1 (PR #268) で日次プラン生成
> (`aiOrchestrator.generatePlan`) から切り出され、自動 cron からは呼ばれません。
> class / プロンプトは将来の再接続 (= Evolution Loop or Top-Level Orchestrator action
> 経由での明示呼出) のために残置されています。本プロンプトは「呼ばれたときの振る舞い」
> を定義するものであり、現在の日次フローには影響しません。
