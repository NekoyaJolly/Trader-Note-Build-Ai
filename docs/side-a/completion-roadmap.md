...
- [ ] リアルタイム類似度 Phase3（レンズ統一→DB/Push/UI 配線）〔L〕 — **常駐ワーカー本番化(δ-5)は当面見送り、15 分 cron 維持（2026-06-13 Neko 決定、§13.4）**。15m 以上の時間足は実質バー単位評価のため、リアルタイム化は 5m 以下が必要になった時点で再判断
- [ ] 通知のリアルタイム UI〔M〕 — **認証付き per-user 通知 SSE を新設**（既存チャート SSE は認証なし・symbol 単位のため流用しない。2026-06-13 Neko 決定、§13.3 δ-3） / Web Push 購読状態 UI〔S〕
