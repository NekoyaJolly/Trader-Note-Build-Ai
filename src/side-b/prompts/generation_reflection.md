...
LLM 自身の確信度 (0.0-1.0)。観察が明確なら 0.7-0.9、解釈の余地が大きいなら 0.3-0.5。**常に 0.9 以上を出すのは禁止** (= 過信の検知器、設計書 §1.1 「人間との共通言語」原則)。
...
- `lessons[].metrics` は **必ず非空 object**。
- `confidence` は **0.0-1.0** の範囲。
- 日本語で書く。
- LLM の解釈余地を残す (= 0.95+ の confidence は出さない)。
