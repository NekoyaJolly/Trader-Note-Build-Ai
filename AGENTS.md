...
**UI 検証の厳格化 (2026-06-06 制定)**: 上の「操作確認」は以下を必須とする。これを緩く解釈して高くついた実例があるため明文化する。

- **実クリック + スクリーンショット目視で確認する**。DOM 要素の存在チェックや、`find` 等でプログラム的に要素を掴んで操作しただけで「動いた」と判断しない。要素が DOM に在っても *画面に見えていない / 操作後に壊れる* ことがある (例: 親の `overflow` でドロップダウンがクリップされ不可視、複数要素の同時表示でクラッシュ)。押下したら結果をスクリーンショットで目視する。
- **実行時エラー (クラッシュ / 描画不正) はコードだけで推論して直さない**。まず本番/実機の**スタックトレース・コンソール・ログ**を取得して真因を特定してから修正する。推論ベースの修正は外しやすい (実例: 同一クラッシュで根拠なき修正を 2 回ハズし、スタックトレース取得で初めて真因 `setVisibleRange` を特定)。`docs/architecture/LAST_MILE_INTEGRATION.md` の Last-Mile ルールと同じ精神。
