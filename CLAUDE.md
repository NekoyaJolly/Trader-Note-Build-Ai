# CLAUDE.md - 自律型トレーディングAI アーキテクチャ

> Claude Code が本プロジェクトで作業する際、毎回参照する前提の短い指針。
> プロジェクト全体のガイドは `AGENTS.md` を参照。
> 完全な設計意図は `docs/design/DESIGN_DOC_autonomous_trading_architecture.md` を参照。
...
不明点が出たら必ず `docs/design/DESIGN_DOC_autonomous_trading_architecture.md` に戻る。
