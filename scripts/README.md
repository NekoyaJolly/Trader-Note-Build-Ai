...
| `check/production-auth-setup.ts` | 本番認証済み smoke 用の Playwright storageState を手動ログインで作成する | `npm run test:e2e:production:auth` | check | GitHub Actions 等で安全な一時認証を発行できる専用 smoke 基盤へ移行した場合 |
| `check/production-ui-walkthrough.ts` | `.auth/production.json` を使い、本番主要 UI が認証済みで表示できることを read-only で確認する | `npm run test:e2e:production:walkthrough` | check | 認証済み Playwright spec に統合され、個別スクリプトが不要になった場合 |
| `check/production-functional-test.ts` | 本番主要 read API と実発注停止ゲートを認証済みで確認する safety smoke | `npm run test:e2e:production:functional` | check | 認証済み golden path E2E に完全統合された場合 |
| `check/production-backtest-test.ts` | 本番ストラテジー一覧とバックテスト画面に到達できることを read-only で確認する | `npm run test:e2e:production:backtest` | check | バックテスト画面の認証済み E2E spec に統合された場合 |
| `check/lens-snapshot-backfill-status.ts` | TradeNote の Note コア行 / lensSnapshot バックフィル未処理件数を read-only で集計する | `npx tsx scripts/check/lens-snapshot-backfill-status.ts [--include-archived] [--fail-on-pending]` | check | LensSnapshot バックフィル完了後、旧 12 次元経路を削除し再生成ニーズが無くなった場合 |
| `check/api-owner-check-todos.ts` | `docs/API.md` に残る owner check `TODO: confirm` 行を一覧化し、実装済み/stale docs/未実装を棚卸しする | `npx tsx scripts/check/api-owner-check-todos.ts [--fail-on-todo]` | check | `docs/API.md` の owner check TODO が全て解消され、CI で docs/API 整合性を別途検証できるようになった場合 |
...
| `migrate/backfill-lens-snapshots.ts` | 既存 TradeNote へ Note コア行 + lensSnapshot をトレード時刻起点で遡及生成 (NOTE_SIMILARITY_FOUNDATION.md §9-5)。lensSnapshot=null の再試行も兼ねる | `npx tsx scripts/migrate/backfill-lens-snapshots.ts --dry-run [--limit N] [--include-archived]` → 書き込み時のみ `--confirm-write` を追加 | migrate | 全既存ノートのバックフィル完了 + マッチング経路切替 (§9-3) 後に再生成ニーズが無くなった場合 |
