...
- [ ] `HypothesisBacktestOrchestrator` が実装され、仮説を受けて materialize → Side-A BT 実行 → 結果を EdgeHypothesis に反映できる
- [ ] `HypothesisWalkForwardOrchestrator` が実装され、同様にウォークフォワードを実行・反映できる
...
### 4.4 HypothesisBacktestOrchestrator
...
4a では backtest/walkForward フィールドの形式がやや抽象的だった。今回の実装で具体化されるため、判定ロジックを正確に実装する。
