...
export class FeatureService {
  ...
  /**
   * 特定のトレードノートに類似するノートを検索
   * 
   * @param noteId - 基準となるトレードノート ID
   * @param topK - 取得件数
   * @param minSimilarity - 最小類似度
   * @returns 類似ノートの配列
   */
  async findSimilarToNote(
    noteId: string,
    topK: number = 10,
    minSimilarity: number = 0.5
  ): Promise<SimilarNoteResult[]> {
    // 基準ノートの特徴量を取得
    const baseNote = await prisma.tradeNote.findUnique({
      where: { id: noteId },
      select: { featureVector: true, symbol: true },
    });

    if (!baseNote || !baseNote.featureVector || baseNote.featureVector.length === 0) {
      return [];
    }
    ...
    if (baseNote.symbol) {
      whereClause.symbol = baseNote.symbol;
    }
    ...
    for (const note of notes) {
      ...
      const similarity = this.cosineSimilarity(baseNote.featureVector, note.featureVector);
      const distance = this.euclideanDistance(baseNote.featureVector, note.featureVector);
