...
- [ ] リアルタイム類似度 Phase3（レンズ統一→DB/Push/UI 配線）〔L〕 — **常駐ワーカー本番化(δ-5)は当面見送り、15 分 cron 維持（2026-06-13 Neko 決定、§13.4）**。15m 以上の時間足は実質バー単位評価のため、リアルタイム化は 5m 以下が必要になった時点で再判断。残 = δ-1（休眠 `realtimeSimilarityService` の簡易12次元をレンズ統一、§13.2）
