...
- **HG が想定している RR 比 2.0(プロンプト推奨)から実勢 RR が上振れしている**:
  - RR 比 2.0 で損益分岐勝率は **33.3%**
  - 実際の勝率中央値 26.2% で PF 1.13 が出ている → 「平均勝ち / 平均負け」が **約 3.18** に達している(2.0 想定より上振れ)
  - **理屈**: HG は RR=2.0 を狙うが、実際の TP/SL ヒット時の利幅は 3.18 倍 → トレンドが強い時に TP まで伸び、伸びすぎて勝率が低くなる典型パターン
...
### 2.3 maxHoldingBars の関係

`maxHoldingBars: 48` × 15m = **12 時間**(デフォルト、minutes 換算で 720 分)。
...
| **C** | maxHoldingBars を 48 → 24 に半減 | timeout 増、勝ち取りこぼし | スキャル寄り |
