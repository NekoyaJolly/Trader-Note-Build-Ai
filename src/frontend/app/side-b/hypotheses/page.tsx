 ...
 *
 * 機能:
 *   - ステータス・カテゴリ・ソース・シンボル・検索・ソートでフィルタ
 *   - ページネーション（既定 20件/ページ）
 *   - フィルタ状態は URL query string と同期（ブックマーク / 直接遷移対応）
 *   - カードクリックで /side-b/hypotheses/[id]（Step 4 で実装）
 *
 * 設計メモ:
 *   - カスタムフックはまだ作らず sideBApi を直接呼ぶ。polling の必要は無い画面。
 *   - "確信度順" は未対応（HypothesisListSortKey は newest/oldest/observation のみ）。
 *   - 失敗時は Alert 風メッセージ + リトライボタン。空状態は専用メッセージ。
 */
...
/**
 * Next.js 16: useSearchParams を使う client component は Suspense で
 * ラップしないとプレレンダリング時に警告が出る（既存 /orders/preset や
 * /auth/ctrader/callback と同じパターン）。
 */
...
function HypothesesListInner() {
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams();

    // URL から初期フィルタを取得（ブラウザ戻る・ブックマーク対応）
    ...
    // URL と filters を双方向同期（filters 更新 → URL push）
    React.useEffect(() => {
        ...
        // pathname, router は安定、searchParams は URL 同期側からのみ変わる
