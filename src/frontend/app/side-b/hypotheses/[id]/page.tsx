 ...
 *
 * Phase 4c 実データ不在の現状を踏まえた設計:
 *   - fullValidationReport が欠けている仮説でも表示崩れしない
 *     (ValidationReport 側で「本格検証未実施」案内を出す)
 *   - LLM 解釈は存在すれば表示・無ければ非表示
 *   - 再検証ボタンは簡易版 (ValidationTrigger 化は Step 5 で)
 *
 * 省略（将来対応）:
 *   - 棄却ボタン (管理者機能、仕様書 §4.3)
 *   - 編集ボタン (仕様書で hidden 指示)
 *   - ブラウザ戻るでの URL→state 同期 (Step 3 と同じ制約)
 *   - レンズスナップショット表示: EdgeHypothesis に該当フィールドが無く
 *     データソースが存在しないため、LensSnapshotView の empty 状態を表示
 */
...
export default function HypothesisDetailPage({ params }: PageParams) {
    // Next.js 16: params は Promise として渡るため use() で unwrap
    const { id } = use(params);
    ...
    const handleRevalidate = async () => {
        ...
        try {
            const result = await sideBApi.triggerValidation(id);
            setValidateMessage({
                type: "success",
                text: `判定: ${result.verdict}`,
                verdict: result.verdict,
            });
            // 完了後に詳細を再取得（ステータスや fullValidationReport が更新される）
            await load();
        } catch (err) {
            const msg = err instanceof SideBApiError ? err.message : String(err);
