...
- 層重みプリセットは `NotificationPreference.weightPreset` に保持する（2026-06-17 実装）。scope=user/profile/note はノートマッチで `note > profile > user > システム既定` の順に解決し、`profile` は `TradeNote.indicatorConfig.profileId` から適用する。`null` は既定 `indicator_focused` を意味する。条件アラートの strategy scope では現時点で `cooldownMinutes` と `maxPerDay` のみを実効値として使い、`weightPreset` は条件ツリーがレンズ類似スコアを直接使う段階まで no-op とする。
